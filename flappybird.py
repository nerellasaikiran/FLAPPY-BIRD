...
import pygame
from pygame.locals import *  # Basic Pyg
...
GAME_SPRITES = {}
GAME_SOUNDS = {}
PLAYER = 'assets\sprites\\bluebird-downflap.png'
...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    ...
    for pipe in lowerPipes:
        if (playery + GAME_SPRITES['player'].get_height() > pipe['y']) and abs(playerx - pipe['x']) < \
                GAME_SPRITES['pipe'][0].get_width() - 20:
            GAME_SOUNDS['hit'].play()
            pygame.mixer.music.stop()
            gameOver()
    ...
    while True:
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pos()[0] > 30 and pygame.mouse.get_pos()[0] < 30 + GAME_SPRITES['RETRY'].get_width():
                if 220 < pygame.mouse.get_pos()[1] < 220 + GAME_SPRITES[
                    'RETRY'].get_height():
                    pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
            ...
            if pygame.mouse.get_pos()[0] > 30 and pygame.mouse.get_pos()[0] < 30 + GAME_SPRITES['HOME'].get_width():
                if pygame.mouse.get_pos()[1] > 280 and pygame.mouse.get_pos()[1] < 280 + GAME_SPRITES[
                    'HOME'].get_height():
                    pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        welcomeScreen()
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('Flappy Bird With Nerella')  # Setting the Caption of The Game
